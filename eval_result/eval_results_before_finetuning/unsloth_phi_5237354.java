...
14:24:10,057 INFO  [STDOUT] ------------------------------------------------------------------------


A: You are using the == operator to compare two Lists. This will only return true if both references are the same.
Use the List.isEmpty() method to check if the list is empty.

A: You are using the == operator to compare two Lists. This will only return true if both references are the same.
Use the List.isEmpty() method to check if the list is empty.

A: You are using the == operator to compare two Lists. This will only return true if both