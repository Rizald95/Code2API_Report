...
static Throwable launch(String cmd, File fil, int lin, int col) throws Throwable {
    ...
    for(int xa=0; xa<arr.length; xa++) {
        if(TextUtil.isQuoted(arr[xa])) {
            arr[xa]=TextUtil.unquote(arr[xa]);
        }
    }
...
A: I'm not sure if this is what you're looking for, but you could use ProcessBuilder to launch the external programs.  ProcessBuilder will launch the process in a separate thread, and you can use the Process.destroy() method to kill the process when the Java program exits.
Here's an example:
