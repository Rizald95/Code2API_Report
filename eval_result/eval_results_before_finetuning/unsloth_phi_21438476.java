...
Here's a complete Java method that reads characters from the standard input, processes each character (skipping newline and carriage return characters), and then continues reading until a negative value is encountered (which is typically -1, indicating the end of input):

```java
import java.io.IOException;
import java.util.Scanner; // Using Scanner for reading from standard input
...
public class CharInputReader {
    ...
    public static void main(String[] args) {
        try (Scanner scanner = new Scanner(System.in)) {
            int ch;
            while ((ch = scanner.next().charAt(0)) != -1) {
