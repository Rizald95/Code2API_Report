...
Here's a version that uses the java.time.Instant class, which is the modern equivalent of Calendar.

    public long returnSecondsSinceEpoch(int year, int month, int date) {
        Instant instant1 = Instant.EPOCH;
        Instant instant2 = Instant.now().minusSeconds(System.currentTimeMillis() % 1000);
        Instant instant3 = Instant.ofEpochDay(year - 1970, month - 1, date).atStartOfDay(ZoneOffset.UTC);
        long seconds = Duration.between(instant1, instant3).