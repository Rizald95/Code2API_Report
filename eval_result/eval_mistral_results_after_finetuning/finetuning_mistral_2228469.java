...
However, it requires to set the initial size to a large enough size, so that the content fits in, or they won't be put on the layout. This is a bit pity, and not a real solution to my question.

Any idea on how to solve it in a cleaner way?
    
Code: JPanel lPanel = new JPanel();
lPanel.setLayout(new BoxLayout(lPanel, BoxLayout.PAGE_AXIS));
...
You can use the method `pack()` on a JFrame containing your JPanel.
