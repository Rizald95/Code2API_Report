...
Answer: 
I'm not sure if this is any better than what you already have. Given that the input is a String[] and the (intermediate) output you want is a String, you could try to use the collect() method with a Collector that accumulates the items into a List, then joins the items in the List into a String.

Here's a complete method that does that:

String joinWithComma(String[] arr) {
    return Arrays.stream(arr)
                .collect(Collectors.joining(","));
}

You can try it here: https://ideone.com/vZKZgx

    
Code: String joinWithComma(String[] arr) {
    return Arrays.stream(arr)
                .collect(Collectors.joining(","));
