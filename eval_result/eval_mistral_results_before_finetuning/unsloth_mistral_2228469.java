...
However, it requires to set the initial size to a large enough size, so that the content fits in, or they won't be put on the layout. This is a bit pity, and not very elegant.

I would like to know if there is a way to do this without setting an initial size, or if there is a better way to do this.

I also tried to use the pack() method, but it doesn't work as expected, as it doesn't adjust the size of the panel, only the size of the frame.

I also tried to use the setSize() method on the frame, but it doesn't work as expected either, as it doesn't adjust the size of the panel, only the size of the frame.

I also tried to use the setSize() method on the panel, but it doesn't work as expected, as it doesn't