...
Code: value=value.replaceAll("([\\*\\+\\?])","\\\\$1");


For the equality check, you can use the equals() method instead of matches() method.

```
if (equality.equals("=")) {
    result = valueFromHeader.equals(value);
} else if (equality.equals("!=")) {
    result = !valueFromHeader.equals(value);
}
```

For the case where you have a property like PropertyC != '*-this', you can use the following regex:

```
valueFromHeader.matches("^[^\\*]*[^\\*]*-this$")
```