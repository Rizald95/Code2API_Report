...
* We have two hour constants, `from` and `to`.
* We create a `Date` object to get the current time.
* We create a `Calendar` object and set its time to the current time.
* We calculate the current time as an integer in the format `HHmm`.
* We check if the current time is between the two