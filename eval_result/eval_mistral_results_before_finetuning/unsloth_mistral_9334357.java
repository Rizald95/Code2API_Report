...
public static void listCipherAlgorithms() {
  for (Provider provider: Security.getProviders()) {
    System.out.println(provider.getName());
    for (String key: provider.stringPropertyNames())
      System.out.println("\t" + key + "\t" + provider.getProperty(key));
...
This method will print out the name of each provider and all of its properties.  You can search for the word "Cipher" or "CipherSpi" in the output to find the list of available Cipher algorithms.