...
class ConnectionTest {
    public static void main(String[] args) {
        try {
            ...
            //use inputLine.toString(); here it would have whole source
            dis.close();
            String data_string = inputLine.toString();
        } catch (MalformedURLException me) {
            System.out.println("MalformedURLException: " + me);
        } catch (IOException ioe) {
            System.out.println("IOException: " + ioe);
...
In this code, we are using DataInputStream to read the data from the URL. We are reading the data line by line and storing it in a StringBuffer. After reading all the lines, we convert the StringBuffer to a String and store