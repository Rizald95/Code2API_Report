...
The `DateTimeFormatterBuilder.getLocalizedDateTimePattern()` method returns a pattern string for the specified format style and locale. The pattern string includes the year, day of year, and week year components, which we don't want to display.

To remove these components, we replace the entire pattern string with a regular expression that matches the desired components (day of week, month, and day) and captures them in groups. The replacement string consists of the captured groups only.

The resulting pattern string will display the desired format for the