...
static Throwable launch(String cmd, File fil, int lin, int col) throws Throwable {
    ...
    for(int xa=0; xa<arr.length; xa++) {
        if(TextUtil.isQuoted(arr[xa])) {
            arr[xa]=TextUtil.stripQuotes(arr[xa]);
        }
    }

    Process pr=Runtime.getRuntime().exec(arr);
    pr.getErrorStream();
    pr.getInputStream();
    pr.getOutputStream();
...
I have tried using ProcessBuilder as suggested.  The code I have now is as follows:
