...
14:24:10,057 INFO  [STDOUT] restrictList= [lower(logs.clazz) like lower(concat(logs.clazz,%)), lower(logs.rule) like lower(concat(logs.rule,%))]


Edit:
Complete method

    protected List<String> constructRestrictionList() {
        List<String> restrictList = new ArrayList<String>();
        restrictList.add("lower(logs.clazz) like lower(concat(#{logs.